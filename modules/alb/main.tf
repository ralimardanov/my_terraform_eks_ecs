...
# Application Load Balancer configuration. It will load balance traffic across all our instances.
resource "aws_alb" "load_balancer" {
  name               = "${var.common_tags["Env"]}-${var.common_tags["Component"]}-ALB"
  ...
  tags                       = merge(var.common_tags, { Name = "${var.common_tags["Env"]}-${var.common_tags["Component"]}-ALB" })
}
resource "aws_alb_target_group" "target_group" {
  name        = "${var.common_tags["Env"]}-${var.common_tags["Component"]}-TG"
  port        = var.tg_port
  ...
  tags       = merge(var.common_tags, { Name = "${var.common_tags["Env"]}-${var.common_tags["Component"]}-TG" })
}
